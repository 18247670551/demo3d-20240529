...
export default class CustomPlan extends THREE.Mesh{
    ...
    constructor(points: number[][][], options?: CustomPlanOptions) {
    ...
    // 分配三角形面顶点
    private static getIndexs(last_i: number, last_j: number, count: number, i: number, j: number) {
    ...
    // 生成顶点颜色
    private static getHue(max: number, min: number, maxHsl: number, minHsl: number, h: number,) {
