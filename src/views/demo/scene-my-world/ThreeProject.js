...
export default class ThreeProject extends ThreeCore {
    ...
    addSkyBox() {
        const rgbeLoader = new RGBELoader().setPath("/demo/scene-my-world/");
    ...
    createMap() {
        ...
        for (let z = 0; z < worldDepth; z++) {
            for (let x = 0; x < worldWidth; x++) {
                let h = getY(x, z);
                matrix.makeTranslation(x * grassBlockUnit - worldWidth / 2 * grassBlockUnit, h * grassBlockUnit, z * grassBlockUnit - worldDepth / 2 * grassBlockUnit);
                let px = getY(x + 1, z);
                let nx = getY(x - 1, z);
                let pz = getY(x, z + 1);
                let nz = getY(x, z - 1);
        ...
        const texture = getTextureLoader().load("public/demo/scene-my-world/textures/blocks/grass.png");
    ...
    generateHeight(grassBlockUnit, width, height) {
        const data = [];
