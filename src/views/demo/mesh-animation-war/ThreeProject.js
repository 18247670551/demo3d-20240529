...
export default class ThreeProject extends ThreeCore {
    ...
    constructor(dom) {
        ...
        getDracoGltfLoader().load("/demo/mesh-animation-war/ew8.glb", (gltf) => {
            ...
            // 俄罗斯
            const els = obj.children[0];
            // 乌克兰
            const wkl = obj.children[1];
            // 导弹飞行轨迹
            const path = obj.children[2];
            // 导弹飞行
            const missile = obj.children[3];
            this.missile = missile;
            this.scene.add(els, wkl, missile);
            // 创建曲线
            const points = [];
            for (let i = path.geometry.attributes.position.count - 1; i >= 0; i--) {
                points.push(new THREE.Vector3(path.geometry.attributes.position.array[i * 3], path.geometry.attributes.position.array[i * 3 + 1], path.geometry.attributes.position.array[i * 3 + 2]));
            }
