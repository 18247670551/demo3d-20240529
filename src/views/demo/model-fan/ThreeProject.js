...
import Fan, { BtnState, Level, LevelBtn, ShakeBtn } from "@/views/demo/model-fan/Fan";
export default class ThreeProject extends ThreeCore {
    ...
    load() {
        new GLTFLoader().load("/demo/model-fan/fan.glb", gltf => {
            ...
            btns.forEach(([name, level]) => {
                ...
                if (btn)
                    this.fan.btns.push(new LevelBtn(btn, level));
            });
            let btn = gltf.scene.getObjectByName("Shake");
            if (btn)
                this.fan.btns.push(new ShakeBtn(btn));
        });
        this.renderer.domElement.addEventListener("pointerdown", (event) => {
            ...
            this.renderer.domElement.addEventListener("pointerup", () => this.fan.handleClick(intersects, BtnState.up), { once: true });
        });
    }
}
