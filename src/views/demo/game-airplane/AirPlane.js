...
export default class AirPlane extends THREE.Group {
    ...
    // 驾驶舱
    addCockpit() {
        const geo = new THREE.BoxGeometry(80, 50, 50, 1, 1, 1);
        const mat = new THREE.MeshPhongMaterial({ color: this.options.colors.red, flatShading: true });
    ...
    addEngine() {
        const geo = new THREE.BoxGeometry(20, 50, 50, 1, 1, 1);
        const mat = new THREE.MeshPhongMaterial({ color: this.options.colors.white, flatShading: true });
        const mesh = new THREE.Mesh(geo, mat);
        mesh.position.x = 40;
        mesh.castShadow = true;
        mesh.receiveShadow = true;
    ...
    addTail() {
        const geo = new THREE.BoxGeometry(15, 20, 5, 1, 1, 1);
        const mat = new THREE.MeshPhongMaterial({ color: this.options.colors.red, flatShading: true });
        const mesh = new THREE.Mesh(geo, mat);
    ...
    // 挡风玻璃
    addWindshield() {
        const goe = new THREE.BoxGeometry(3, 15, 20, 1, 1, 1);
